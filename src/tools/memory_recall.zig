...
pub const MemoryRecallTool = struct {
    ...
    pub const tool_params =
        \\{"type":"object","properties":{"query":{"type":"string","description":"Keywords or phrase to search for in memory"},"limit":{"type":"integer","description":"Max results to return (default: 5)"},"depth":{"type":"string","enum":["abstract","overview","full"],"description":"Level of detail: abstract (brief, default), overview (medium), full (complete content)"}},"required":["query"]}
    ;
    ...
    pub fn execute(self: *MemoryRecallTool, allocator: std.mem.Allocator, args: JsonObjectMap) !ToolResult {
        ...
        const depth = mem_root.ContextDepth.fromString(root.getString(args, "depth") orelse "abstract");
        ...
        return formatEntries(allocator, entries, visible_entries, depth);
    }
    ...
    fn formatEntries(allocator: std.mem.Allocator, entries: []const MemoryEntry, visible_count: usize, depth: mem_root.ContextDepth) !ToolResult {
        ...
        for (entries, 0..) |entry, i| {
            ...
            try buf.appendSlice(allocator, "): ");
            try buf.appendSlice(allocator, entry.atDepth(depth));
