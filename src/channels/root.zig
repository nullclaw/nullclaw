...
pub const Channel = struct {
    ...
    /// A user decision requested by the agent for a potentially unsafe action.
    /// `request_id` is generated by the agent and must be echoed unchanged by
    /// the client in its approval response.
    ...
    pub const VTable = struct {
        ...
        /// Optional structured approval delivery. Channels must preserve the
        /// request id as a top-level correlation id on their native protocol.
        sendApprovalRequest: ?*const fn (
            ptr: *anyopaque,
            target: []const u8,
    ...
    pub fn sendApprovalRequest(self: Channel, target: []const u8, request: ApprovalRequest) !void {
        if (self.vtable.sendApprovalRequest) |fn_send_approval_request| {
            return fn_send_approval_request(self.ptr, target, request);
        }
        return error.NotSupported;
    }
...
test "Channel message operations default to not supported" {
    ...
    try std.testing.expectError(error.NotSupported, channel.sendApprovalRequest("chat-1", .{
        .request_id = "req-1",
...
test "Channel structured approval request dispatches through vtable" {
    const Mock = struct {
        ...
        fn sendApprovalRequest(ptr: *anyopaque, target: []const u8, request: Channel.ApprovalRequest) anyerror!void {
            const self: *@This() = @ptrCast(@alignCast(ptr));
            self.target = target;
    ...
    var mock = Mock{};
    const channel = Channel{ .ptr = @ptrCast(&mock), .vtable = &Mock.vtable };
    try channel.sendApprovalRequest("chat-2", .{
        .request_id = "req-2",
        .action = "delete file",
        .reason = "destructive operation",
    });
    try std.testing.expectEqualStrings("chat-2", mock.target.?);
    try std.testing.expectEqualStrings("req-2", mock.request_id.?);
    try std.testing.expectEqualStrings("delete file", mock.action.?);
    try std.testing.expectEqualStrings("destructive operation", mock.reason.?);
}
