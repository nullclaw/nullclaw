...
| `/pair` | POST | `X-Pairing-Code` | Exchange one-time pairing code for bearer token (loopback-only when gateway is publicly bound). The issued token is also persisted (encrypted at rest when `secrets.encrypt = true`) to `paired_token` in the config dir so the schedule/cron tool can authenticate to `/cron` |
...
7. `/pair` persists the issued bearer token to `paired_token` in the config directory (encrypted with the shared `SecretStore` when `secrets.encrypt = true`, mode `0600`) so the schedule/cron tool can read and use it to call `/cron/*`. `/logout` removes this file on revocation. This is required for `require_pairing = true` to work with the scheduler; without it the cron tool has no way to obtain a token and `/cron` returns `401`.
